...
def anchor_target_layer(rpn_cls_score, gt_boxes, im_info, _feat_stride, all_anchors, num_anchors):
  ...
  # map of shape (..., H, W)
  height, width, depth = rpn_cls_score.shape[1:4]
  ...
  overlaps = bbox_overlaps(
    ...
    np.ascontiguousarray(gt_boxes, dtype=np.float))  # need to call third-party function
  ...
  # labels
  labels = labels.reshape((1, height, width, depth, A)).transpose(0, 4, 1, 2, 3)
  labels = labels.reshape((1, 1, A * height, width, depth))
  rpn_labels = labels

  # bbox_targets
  bbox_targets = bbox_targets \
    .reshape((1, height, width, depth, A * 6))
